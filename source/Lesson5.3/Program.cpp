...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE, LPSTR, int showCommand)
{
	...
	//Sets up the Window to display the application
	const wstring windowClassName = L"RenderingClass"s;
	const wstring windowTitle = L"Our Lovely Solar System"s;
	...
	//Sets up the RenderingGame with the created window, then initializing it
	RenderingGame game(getWindow, getRenderTargetSize);
	game.UpdateRenderTargetSize();
	game.Initialize();

	//This will execute as long as the game has not been quit by the user
	MSG message{ 0 };
	try
	{
		while (message.message != WM_QUIT)
		{
			//Checks for messages, if so, translating and carrying them out
			...
			{
				//Otherwise, runs the game normally
				game.Run();
			}
		}
	}
	//Works to catch exceptions and respond appropriately
	catch (GameException ex)
	{
		MessageBox(windowHandle, ex.whatw().c_str(), windowTitle.c_str(), MB_ABORTRETRYIGNORE);
	}

	//Shuts down the game and cleans up potential memory leak areas before ending
	game.Shutdown();
